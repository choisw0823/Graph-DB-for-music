...
#print(N.g.V().hasLabel('audio_feature').valueMap().filter_(lambda vertex: cosine_similarity(vertex, audio_feature, 0.6)))
#exit()
...
if inp['audio_feature']['instrument'] is not None: #and inp['track']['name'] is None and inp['track']['id'] is None:
    for track in N.g.V().hasLabel('track').values('id').toList():
        ...
        if get_similarity(N.g.V().hasLabel('track').has('id', track).out('track_instrument').values('name').toList(), inp['audio_feature']['instrument'], mapping) > threshold:
            if result is None:
                result = set(N.g.V().hasLabel('track').has('id', track).inE('artist_track').outV().out('artist_event').inE('venue_event').outV().dedup().values('id').toList())
            else:
                result = result | set(N.g.V().hasLabel('track').has('id', track).inE('artist_track').outV().out('artist_event').inE('venue_event').outV().dedup().values('id').toList())
